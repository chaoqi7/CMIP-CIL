...
from convs.pointnet_cls import pointnet #for MEMO cifar
from convs import CrossModel
...
def get_convnet(args, pretrained=False):
    ...
    if name == "resnet32":
        ...
        return CrossModel.PCS_Mviews_CrossModel(args)
    ...
    # PointCloud benchmark backbone
    elif name == "pointnet":
        return pointnet(pretrained=pretrained,args=args)
    elif name == "pointnet2":
        return pointnet(pretrained=pretrained,args=args)
...
class BaseNet(nn.Module):
    ...
    def extract_vector(self, x, x2, x_mviews):
        x_mviews = x_mviews.flatten(0, 1)
        point_feats1, point_feats2, img_feats, layers_feats = self.convnet(x, x2, x_mviews)
        return img_feats

    def forward(self, x, x2, x_mviews, layerout = False ,test=False):
        point_feats, point_feats2, img_feats, layers_feats = self.convnet.forward(x, x2, x_mviews, layerout = layerout, test=test)

        if test == False:
            out = self.fc(img_feats)
        else:
            out = self.fc(point_feats)

        # out = self.fc(img_feats)  # shiyan

        out.update({"point_feats": point_feats})
        out.update({"point_feats2": point_feats2})
        out.update({"img_feats": img_feats})
        if layerout == True:
            out.update({"layers_feats": layers_feats})
...
class FOSTERNet(nn.Module):
    ...
    def extract_vector(self, x, x2, x_mviews):
        ...
        for convnet in self.convnets:
            point_feats, point_feats2, img_feats, layers_feats = convnet.forward(x, x2, x_mviews)
            features.append(img_feats)
        features = torch.cat(features, 1)
        return features

    def forward(self, x, x2, x_mviews, test=False):
        features = []
        for convnet in self.convnets:
            point_feats, point_feats2, img_feats, layers_feats = convnet.forward(x, x2, x_mviews)
            if test == False:
    ...
    def forward(self, x, x2, x_mviews, test=False):
        ...
        if self.old_fc is None:
            ...
            for i in range(len(self.task_sizes)-2,-1,-1):
                ...
                assert not torch.isnan(new_fc_weight).any()
                assert not torch.isnan(new_fc_bias).any()
