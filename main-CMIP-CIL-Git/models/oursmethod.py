...
class OursMethod(BaseLearner):
    ...
    def replace_fc(self, trainloader: object, model: object, args: object, optimizer: object, scheduler: object) -> object:
        model = model.eval()
        embedding_list = []
        label_list = []
        with torch.no_grad():
    ...
    def _train(self, train_loader, test_loader, train_loader_for_protonet):
        ...
        self.replace_fc(train_loader_for_protonet, self._network, None, optimizer, scheduler)
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, inputs2, inputs_mviews, targets) in enumerate(train_loader):
                ...
                ###Loss reg###
                reg_weight = self._network.fc.weight.data
                reg_embed = torch.zeros_like(reg_weight)

                class_list = np.unique(targets.cpu().numpy())
                for class_index in class_list:
                    data_index = (targets == class_index).nonzero().squeeze(-1)
                    embedding = output["img_feats"][data_index]
                    proto = embedding.mean(0)
                    reg_embed[class_index] = proto
                mask = (reg_embed.sum(dim=1) != 0).float().view(-1, 1)
                loss_2 = 1 - (nn.functional.cosine_similarity(reg_weight, reg_embed, dim=1) * mask).mean() # 500 * ((reg_weight - reg_embed) ** 2 * mask).mean()
                loss = loss_1  + loss_2
                ##############
                optimizer.zero_grad()
                loss.backward()
            ...
            if (epoch + 1) % 5 == 0:
                ...
                info = "T {}, Ep {}/{} => Ls1 {:.3f}, Ls2 {:.3f}, Ls {:.3f}, Tra_acc {:.2f}".format(
                    self._cur_task,
                    epoch + 1,
                    self.args['init_epoch'],
                    losses_1 / len(train_loader),
                    losses_2 / len(train_loader),
                    losses / len(train_loader),
                    train_acc,
                )
