...
class FOSTER(BaseLearner):
    ...
    def before_task(self):
        ...
        for name, p in self._network.fe_fc.named_parameters():
            p.requires_grad = True
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, inputs2, inputs_mviews, targets) in enumerate(train_loader):
                inputs, inputs2, inputs_mviews, targets = inputs.to(self._device, non_blocking=True), inputs2.to(
                    self._device, non_blocking=True), inputs_mviews.to(self._device, non_blocking=True), targets.to(
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, inputs2, inputs_mviews, targets) in enumerate(train_loader):
                ...
                logits, fe_logits, old_logits = (
                    ...
                    F.softmax(outputs["fe_logits"]),
                    F.softmax(outputs["old_logits"]).detach(),
                )
    ...
    def _feature_compression(self, train_loader, test_loader):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, inputs2, inputs_mviews, targets) in enumerate(train_loader):
                inputs, inputs2, inputs_mviews, targets = inputs.to(self._device, non_blocking=True), inputs2.to(
                    ...
                    self._device, non_blocking=True)
                inputs_mviews = inputs_mviews.flatten(0, 1)
                output = self._snet(inputs, inputs2, inputs_mviews)
                dark_logits = F.softmax(output["logits"])

                with torch.no_grad():
                    outputs = self._network(inputs, inputs2, inputs_mviews)
                    logits, old_logits, fe_logits = (
                        F.softmax(outputs["logits"]),
                        F.softmax(outputs["old_logits"]),
                        F.softmax(outputs["fe_logits"]),
                    )
        ...
        self._snet.eval()
        y_pred, y_true = [], []
        for _, (_, inputs, inputs2, inputs_mviews, targets) in enumerate(test_loader):
            inputs, inputs2, inputs_mviews, targets = inputs.to(self._device, non_blocking=True), inputs2.to(
                self._device, non_blocking=True), inputs_mviews.to(self._device, non_blocking=True), targets.to(
                self._device, non_blocking=True)
            inputs_mviews = inputs_mviews.flatten(0, 1)
            ...
            with torch.no_grad():
                outputs = F.softmax(self._snet(inputs, inputs2, inputs_mviews, test = True)["logits"])
            predicts = torch.topk(
        ...
        logging.info("CNN top5 curve: {}".format(cnn_accy["top2"]))
