    ...
    def _run(self, train_loader, test_loader, optimizer, scheduler, epk):
        for epoch in range(1, epk + 1):
            ...
            for i, (_, inputs, inputs2, inputs_mviews, targets) in enumerate(train_loader):
                ...
                outputs = self._network(inputs, inputs2, inputs_mviews, layerout = True, test=False)
                logits = F.softmax(outputs["logits"])
                features = outputs["img_feats"]
                fmaps = outputs["layers_feats"]
                ...
                if self._old_network is not None:
                    with torch.no_grad():
                        old_outputs = self._old_network(inputs, inputs2, inputs_mviews, layerout = True, test=False)
                    old_features = old_outputs["img_feats"]
                    old_fmaps = old_outputs["layers_feats"]
                    flat_loss = (
