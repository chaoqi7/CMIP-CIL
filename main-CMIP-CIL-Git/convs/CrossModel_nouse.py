...
class PCS_Mviews_CrossModel(nn.Module):
    ...
    def get_new_adapter(self):
        ...
        self.cur_adapter2 = None
        self.cur_adapter2 = Adapter().cuda()
        self.cur_adapter2.requires_grad_(True)
    ...
    def forward(self, x, x2, x_mviews, layerout = False , test = False):
        ...
        if self.args["pretrain"] == True:# 预训练
            ...
            adapt2 = self.cur_adapter2
            point_feats1 = adapt2(adapt(point_feats1))
            point_feats2 = adapt2(adapt(point_feats2))
            img_feats = adapt2(adapt(img_feats))
            return point_feats1, point_feats2, img_feats, layers_feats
            '''
            if not test:
                adapt = self.cur_adapter
