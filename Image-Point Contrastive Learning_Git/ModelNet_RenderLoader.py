...
def ModelNet40Render(split, maskid = 19):
    ...
    if maskid == -1:
        ...
        output_dir = "/root/autodl-tmp/DataSet/ModelNet40_Multimodals_RandMask/"
    ...
    if process: #需处理文件
        ...
        for index in tqdm(range(len(data_list)), total=len(data_list)):
            ...
            Point_Verts = torch.from_numpy(trimesh.Trimesh(vertices=verts, faces=faces).sample(nb_points, False))
            '''
            indices = np.random.choice(verts.shape[0], size=min(verts.shape[0], nb_points), replace=False)
            Point_Verts = verts[indices]
            ...
            mvtn = MVTN(self.nb_views, views_config="spherical",
